...
# Уникальное имя для безопасного поиска и обновления
...
def run_seed():
    ...
    db.add(r1)
    db.commit()
    q1 = Question(round_id=r1.id, sequence=1, text="O'zbekiston Respublikasining poytaxti qaysi shahar?", points=1)
    db.add(q1)
    db.commit()
    db.add_all([
    ...
    db.add(r2)
    db.commit()
    q2 = Question(round_id=r2.id, sequence=1, text="Yer sayyorasi Quyosh atrofida aylanadi.", points=1)
    db.add(q2)
    db.commit()
    ...
    db.add(r3)
    db.commit()
    q3 = Question(round_id=r3.id, sequence=1, text="Daraxtda o'sadigan shirin, dumaloq, xalq ertaklarida sehrli hisoblangan meva.", points=2)
    q4 = Question(round_id=r3.id, sequence=2, text="Choyga qo'shib ichiladigan nordon sariq sitrus mevasi.", points=2)
    db.add_all([q3, q4])
    db.commit()
    ...
    q5 = Question(round_id=r4.id, sequence=1, text="Kunduz kuni osmonda charog'on nur sochib turuvchi ulkan yulduz.", points=1)
    db.add(q5)
    db.commit()
    db.add(AcceptedAnswer(question_id=q5.id, answer_text="quyosh", is_primary=True))
    
    q6 = Question(round_id=r4.id, sequence=2, text="Aholisi qishloq xo'jaligidan boshqa sohalarda band bo'lgan yirik ma'muriy markaz.", points=1)
    db.add(q6)
    db.commit()
    db.add(AcceptedAnswer(question_id=q6.id, answer_text="shahar", is_primary=True))

    q7 = Question(round_id=r4.id, sequence=3, text="Farg'ona vodiysida joylashgan qadimiy kulolchilik va hunarmandchilik maskani.", points=1)
    db.add(q7)
    db.commit()
    ...
    db.commit()
    db.close()
    print("Muvaffaqiyatli yakunlandi! 1 ta Quiz, 1 ta Versiya, 4 ta Raund va 7 ta Savol kiritildi.")
